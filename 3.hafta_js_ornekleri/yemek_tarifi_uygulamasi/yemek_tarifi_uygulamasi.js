...
async function rastgeleYemekGetir() {
    const sonuc = await fetch("https://www.themealdb.com/api/json/v1/1/random.php");
...
async function yemekIdGetir(id) {
    const sonuc = await fetch("https://www.themealdb.com/api/json/v1/1/lookup.php?i=" + id);
    const sonucBilgisi = await sonuc.json();
    return sonucBilgisi.meals[0];
}

async function arananYemek(gelen) {
    const sonuc = await fetch("https://www.themealdb.com/api/json/v1/1/search.php?s=" + gelen);
    const sonucBilgisi = await sonuc.json();
    return sonucBilgisi.meals;
}
...
function yemekEkle(yemekBilgi, random = false) {
    ...
    btn.addEventListener("click", (e) => {
        e.stopPropagation(); // Popup açılmasını engelle
...
function YemekBilgileriniGoster(yemekBilgi) {
    ...
    for (let i = 1; i <= 20; i++) {
        if (yemekBilgi["strIngredient" + i]) {
            ingredients.push(`${yemekBilgi["strIngredient" + i]} - ${yemekBilgi["strMeasure" + i]}`);
        } else {
            break;
        }
    }
...
searchBtn.addEventListener("click", async () => {
    ...
    if (yemeks) {
        yemeks.forEach(item => yemekEkle(item));
    } else {
        yemekler.innerHTML = "<p style='text-align:center'>Aradığınız tarif bulunamadı!</p>";
    }
});
